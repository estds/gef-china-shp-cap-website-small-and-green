...
const jsonURL = assetsURL + '/data/all-content-v12.9.json';
...
function createSectionDemoPlants(json) {
  ...
  sectionDOM.innerHTML = `<div class="leaflet-map-fullscreen container-fluid d-flex px-0 overflow-hidden"><div class="demo-plant-list-wrap d-flex flex-column align-items-start bg-unido-blue"><h5 class="py-3 px-4 text-white mb-0 d-none d-lg-block">${translation.plantList}</h5><button class="demo-plant-list-toggle d-flex align-items-center justify-content-between text-white w-100 collapsed btn btn-link text-decoration-none py-3 px-4 fs-5 d-lg-none" type="button" data-bs-toggle="collapse" data-bs-target="#demo-plant-list-content" aria-expanded="false">${translation.plantList}</button><div class="collapse demo-plant-list-content bg-dark mt-auto w-100 normalScroll" id="demo-plant-list-content"><ul class="list-group list-group-flush"></ul></div></div><div class="leaflet-map-canva w-100 vh-100 flex-grow-1" id="${json.id}-leaflet-map"></div><div class="leaflet-map-attribution text-light text-opacity-50 text-very-small"><a href="#" data-term-expl="Powered by Leaflet.js with basemap by AutoNavi." data-bs-toggle="modal" data-bs-target="#kh-term-exp" class="text-white small" title="Attribution"><i class="bi bi-info-circle"></i></a></div></div>`;
  ...
  demoPlants.forEach(function(item, index) {
    ...
    listItem.innerHTML = `<a class="d-block text-truncate text-decoration-none text-light px-4" ds-mo-toggle="map-popup" ds-mo-target="$ll-popup-${item.id}" href="#">${item.name}<br><small class="text-secondary">${item.county}</small></a>`
    //
